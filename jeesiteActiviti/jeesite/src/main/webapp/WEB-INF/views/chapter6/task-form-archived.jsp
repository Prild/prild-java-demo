	...
	<p class="task-infos text-left">
		...
		<c:if test="${not empty parentTask}">
			...
			<c:if test="${empty parentTask.endTime}">
				<div>
					该任务属于：<a href="${ctx}/act/task2/getform/${parentTask.id}">${parentTask.name}</a>
				</div>
			</c:if>
			...
			<c:if test="${not empty parentTask.endTime}">
				<div>
					该任务属于：<a href="${ctx}/act/task2/archived/${parentTask.id}">${parentTask.name}</a>
				</div>
			</c:if>
		</c:if>
	</p>
		...
		<ul class="unstyled">
			...
			<li>
				...
				<c:if test="${empty subTask.endTime}">
					<a href="${ctx}/act/task2/getform/${subTask.id}" class="task-name">${subTask.name}</a>
				</c:if>

				<c:if test="${not empty subTask.endTime}">
					<a href="${ctx}/act/task2/archived/${subTask.id}" class="task-name">
