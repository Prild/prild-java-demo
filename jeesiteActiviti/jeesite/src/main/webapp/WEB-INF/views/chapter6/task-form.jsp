...
<%-- *******************************************加载顺序一定要先jquery.js因为其他js都是依赖jquery的******************************************* --%>
...
<script type="text/javascript">
	...
	var ctx = "${ctx}";
	var curruntLoginUser = '${curruntLoginUser}';
</script>
<script type="text/javascript" src="${ctxCss}/common/chapter12/events.js"></script>
<script type="text/javascript" src="${ctxCss}/common/chapter12/task-form.js"></script>
</head>
<body>
	...
	<p class="task-infos text-left">
		<span><i class="icon-calendar"></i>到期日： <span class='due-date'>
				<c:if test="${empty task.dueDate}">无到期日</c:if> <c:if
					test="${not empty task.dueDate}">
					<fmt:formatDate value="${task.dueDate}" pattern="yyyy-MM-dd" />
				</c:if>
		</span> <input type="text" style="display: none"
			class='due-date-input datepicker input-small'
			data-date-format="yyyy-mm-dd" /> </span> <span><i class="icon-flag"></i>优先级：
			<span class='priority'> <c:if test="${empty task.priority}">无到期日</c:if>
		...
		</select> </span> <span><i class="icon-calendar"></i>创建日期： <fmt:formatDate
				value="${task.createTime}" pattern="yyyy-MM-dd hh:mm:ss" /> </span>
	...
	<a href="#addPeopleModal" class="btn btn-small btn-success"
		data-toggle="modal" style="margin-left: 4em;"><i class="icon-plus"></i>邀请参与人</a>

	<%-- 添加候选人/组只能针对未签收的任务 --%>
	<c:if test="${empty task.assignee}">
		<a href="#addCandidateModal" class="btn btn-small btn-success"
	...
	<div class="row">
		<div class="span4">
			<span><i class="icon-user"></i>拥有人(owner)：<span id="owner">${empty
					task.owner ? '无' : task.owner}</span></span> <select id="ownerSelect"
		...
		<div class="span4">
			<span><i class="icon-user"></i>办理人(assignee)：<span id="assignee">${empty
					task.assignee ? '无' : task.assignee}</span></span> <select id="assigneeSelect"
				style="display: none; width: 100px;">
				<c:forEach items="${users}" var="user">
					<option value="${user.loginName}">${user.name}</option>
				</c:forEach>
		...
		<div class="span4">
			...
			<c:if test="${empty task.delegationState}">
				<span><i class="icon-user"></i>任务委派(delegateState)：<span id="delegateState">${empty
						task.delegationState ? '无' : task.delegationState }</span></span>
			</c:if>

			<%-- 已委派 --%>
			<c:if test="${not empty task.delegationState}">
				<i class="icon-user"></i>任务委派：
                    <span id="delegateState"> <c:if
						test="${task.delegationState == 'PENDING' }">
						<i class="icon-ok"></i>被委派
                    </c:if> <c:if test="${task.delegationState == 'RESOLVED' }">
	...
	<div class="row">
		<div class="span">
			<p style="padding-top: 1em;" class="text-info text-center">
				参与人<br />候选[人|组]
			</p>
		</div>
		<div class="span10">
			<ul style="margin-top: 1em;">
				<c:forEach items="${identityLinksForTask}" var="link">
					...
					<c:choose>
						...
						<c:when test="${link.type == '1'}">
							<c:set var="type" value="贡献人（参与人）" />
						</c:when>
						<c:when test="${link.type == '2'}">
							<c:set var="type" value="项目经理（参与人）" />
						</c:when>
						<c:when test="${link.type == '3'}">
							<c:set var="type" value="总经理（参与人）" />
						</c:when>
						<c:when test="${link.type == '4'}">
							<c:set var="type" value="业务顾问（参与人）" />
						</c:when>
						<c:when test="${link.type == '5'}">
							<c:set var="type" value="技术顾问（参与人）" />
						</c:when>
						<c:when test="${link.type == '6'}">
							<c:set var="type" value="执行人（参与人）" />
						</c:when>
					</c:choose>
					<c:if test="${not empty type}">
						<li><span>${empty link.userId ? link.groupId :
								link.userId} -- ${type}</span> <a href="#" data-userid="${link.userId}"
							data-groupid="${link.groupId}" data-type="${link.type}"
							class="link-delete" style="margin-left: 2em;"><i
	...
	<a href="#addSubtaskModal" style="margin-left: 3em;"
		data-toggle="modal" class="btn btn-small btn-success"><i
	...
	<ul class="unstyled">
		<c:forEach items="${subTasks}" var="subTask">
			<li>
				...
				<c:if
					test="${empty subTask.endTime}">
					<a href="${subTask.id}" class="task-name">${subTask.name}</a>
				</c:if> 
				<c:if test="${not empty subTask.endTime}">
					<a href="${ctx}/act/task2/archived/${subTask.id}"
						class="task-name"> ${subTask.name}（完成时间：<fmt:formatDate
							value="${subTask.endTime}" pattern="yyyy-MM-dd hh:mm:ss" />）
					</a>
				</c:if> 
				<c:if test="${subTask.owner == curruntLoginUser && empty subTask.endTime}">
					<a href="javascript:;" style="margin-left: 1em;"
						class="subtask-delete" data-taskid="${subTask.id}"><i
	...
	<div>
		...
		<a href="#addAttachmentModal" style="margin-left: 3em;"
			data-toggle="modal" class="btn btn-small btn-success"><i
			class="icon-plus"></i>添加</a>
		<ul class="unstyled">
			<c:forEach items="${attachments}" var="attachment">
				<li>
					<%-- url类型的文件 --%> <c:if test="${attachment.type == 'url'}">
						<a href="${attachment.url}" target="_blank" data-toggle="tooltip"
							title="地址：${attachment.url}<br/>描述：${attachment.description}"
							data-placement="right" class="attachment-item"><i
							class="icon-share"></i>${attachment.name}</a>
					</c:if> <%-- 文件类型的文件 --%> <c:if test="${attachment.type != 'url'}">
						<a href="${ctx}/act/task3/attachment/download/${attachment.id}"
							target="_blank" data-toggle="tooltip"
							title="${attachment.description}" data-placement="right"
							class="attachment-item"><i class="icon-file"></i>${attachment.name}</a>
					</c:if> <a href="javascript:;" data-id="${attachment.id}"
					class="attachment-delete"><i class="icon-remove"></i>删除</a>
				</li>
			</c:forEach>
		</ul>
	</div>
	...
	<hr />
	<form action="${ctx}/act/task2/complete/${task.id}"
		...
		<c:if test="${!hasFormKey}">
			<c:forEach items="${taskFormData.formProperties}" var="fp">
				...
				<div class="control-group">
					<%-- 文本或者数字类型 --%>
					<c:if
						test="${fp.type.name == 'string' || fp.type.name == 'long' || fp.type.name == 'double'}">
						<label class="control-label" for="${fp.id}">${fp.name}:</label>
						<div class="controls">
							<input type="text" id="${fp.id}" name="${fp.id}"
								data-type="${fp.type.name}" value="${fp.value}" ${readonly}
								${required} />
						</div>
					</c:if>
					...
					<c:if test="${fp.type.name == 'date'}">
						...
						<div class="controls">
							<input type="text" id="${fp.id}" name="${fp.id}"
								class="datepicker" value="${fp.value}"
								data-type="${fp.type.name}"
								data-date-format="${fn:toLowerCase(datePattern)}" ${readonly}
								${required} />
						</div>
					</c:if>
		...
		<div class="control-group">
			<div class="controls">
				...
				<c:if test="${empty task.assignee}">
					<a class="btn"
						href="${ctx }/act/task2/claim/${task.id}?nextDo=handle"><i
						class="icon-ok"></i>签收</a>
				</c:if>
			</div>
		</div>
	</form>
	...
	<div class="row">
		<div class="span2">
			...
			<fieldset id="commentContainer" style="margin-left: 5em;">
				...
				<button id="saveComment" type="button" class="btn">
					<i class="icon-plus"></i>保存意见<i class="icon-comment"></i>
				</button>
			</fieldset>
		</div>
		<div class="span10">
	...
	<!-- 添加参与人对话框 -->
	<div id="addPeopleModal" class="modal hide fade" tabindex="-1"
		role="dialog" aria-labelledby="addPeopleModalLabel" aria-hidden="true">
		...
		<div class="modal-body">
			<table id="participantsTable" class="table table-hover">
				<thead>
					<tr>
						<th><button class="btn btn-small link-add">
								<i class="icon-plus"></i>
							</button></th>
						<th>人员</th>
						<th>职能</th>
				...
				<tbody>
					<tr>
						<td width="10">1</td>
						<td><select class='user' style="width: 130px">
								<c:forEach items="${users}" var="user">
									<option value="${user.loginName}">${user.name}</option>
								</c:forEach>
						</select></td>
						<td><select class="link-type" style="width: 100px;">
						...
						<td>
							<button class="btn btn-small link-remove">
	...
	<!-- 添加候选[人|组]对话框 -->
	<div id="addCandidateModal" class="modal hide fade" tabindex="-1"
		role="dialog" aria-labelledby="addCandidateModalLabel"
		aria-hidden="true">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal"
				aria-hidden="true">×</button>
		...
		<div class="modal-body">
			<table id="candidateTable" class="table table-hover">
				<thead>
					<tr>
						<th><button class="btn btn-small candidate-add">
								<i class="icon-plus"></i>
							</button></th>
						<th>候选类型</th>
						<th>人/组</th>
				...
				<tbody>
					<tr>
						<td width="10">1</td>
						<td><select class='candidateType' style="width: 100px">
						...
						</select> <select class='group' style="width: 130px; display: none;">
								<c:forEach items="${groups}" var="group">
									<option value="${group.enname}">${group.name}</option>
								</c:forEach>
						</select></td>
						<td>
							<button class="btn btn-small link-remove">
	...
	<!-- 添加子任务对话框 -->
	<div id="addSubtaskModal" class="modal hide fade" tabindex="-1"
		role="dialog" aria-labelledby="addSubtaskModalLabel"
		aria-hidden="true">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal"
				aria-hidden="true">×</button>
		...
		<div class="modal-body">
			<form id="subTaskForm"
				action="${ctx}/act/task2/subtask/add/${task.id}"
				class="form-horizontal" method="post">
				<div class="control-group">
	...
	<!-- 添加附件对话框 -->
	<div id="addAttachmentModal" class="modal hide fade" tabindex="-1"
		role="dialog" aria-labelledby="addAttachmentModalLabel"
		aria-hidden="true">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal"
				aria-hidden="true">×</button>
		...
		<div class="modal-body">
			...
			<div id="my-tab-content" class="tab-content">
				...
				<div id="web" class="tab-pane">
					<form id="urlAttachmentForm"
						action="${ctx}/chapter12/attachment/new/url"
						class="form-horizontal" method="post">
						<input type="hidden" name="taskId" value="${task.id}" /> <input
							type="hidden" name="processInstanceId"
							value="${task.processInstanceId}" />
