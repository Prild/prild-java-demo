			...
			<li class="unBr">
				<aside class="noter-pic">
					<c:if test="${empty user.picImg }">
						<img width="50" height="50" class="picImg" src="${ctx_s }/static_/inxweb/img/avatar-boy.gif">
					</c:if>
					<c:if test="${!empty user.picImg }">
						...
						<li>
							...
							<div class="of mt5">
								<span class="fr"><font class="fsize12 c-999 ml5"><fmt:formatDate value="${ac.addTime}" pattern="yyyy/MM/dd HH:mm" /></font></span> <span class="fl"> <a class="noter-dy vam" title="回复" href="javascript: void(0)" onclick="queryCommentReply('${ac.commentId}')">
										<em class="icon18">&nbsp;</em>(<span class="replyCount${ac.commentId}">${ac.replyCount}</span>)${ac.replyCount}//可以实现实时更新
									</a> <tt title="赞一下" class="noter-zan vam ml10" onclick="addPraise('${ac.commentId}',4)">
										<em class="icon18">&nbsp;</em>(<span class="addPraise${ac.commentId}_4 praiseCount"></span>)
										${ac.praiseCount}//js有问题
									</tt>
								</span>
							</div>
							<div class="n-reply">
