...
<html>
<head>
<meta name="decorator" content="edudefault"/>
<title>课程列表</title>
<script type="text/javascript" src="${ctx_s}/static_/inxweb/front/course.js"></script>
</head> 
<body> 
		...
		<section class="container">
			...
			<section class="c-sort-box">
				<section class="c-s-dl">
						...
						<dd class="c-s-dl-li">
							<ul class="clearfix"><%--所有父专业 --%>
								...
								<c:forEach items="${subjectList}" var="subject">
									<li <c:if test="${queryCourse.subjectId==subject.subjectId or subjectParentId==subject.subjectId}">class="current"</c:if>>
										<a onclick="submitForm(1,${subject.subjectId})" title="${subject.subjectName}" href="javascript:void(0)">${subject.subjectName}
										</a>
									</li>
								</c:forEach>
							</ul>
					...
					<%
					Object liObject =  request.getAttribute("sonSubjectList");
					if(liObject!=null ){
								System.out.println(liObject.toString());
					...
					%>
					<c:if test="${not empty sonSubjectList && sonSubjectList.size()>0 }">
                      	<dl>
							<dt>
						...
						<dd class="c-s-dl-li">
							<ul class="clearfix">
								...
								<c:forEach items="${teacherList}" var="teacher">
									<li <c:if test="${teacher.id==queryCourse.teacherId}">class="current"</c:if>>
										<a title="${teacher.name}" onclick="submitForm(2,${teacher.id_})" href="javascript:void(0)">${teacher.name}
										</a>
									</li>
								</c:forEach>
							</ul>
				...
				<div class="js-wrap">
					<section class="fr">
						<span class="c-ccc"> <tt class="c-master f-fM">${page.pageNum}</tt>/<tt class="c-666 f-fM">${page.pages}</tt>
						</span>
					</section>
					<section class="fl">
						<ol class="js-tap clearfix">
							<li <c:if test="${queryCourse.order=='FOLLOW'}">class="current bg-orange"</c:if>>
								<a title="关注度" onclick="submitForm(3,'FOLLOW')" href="javascript:void(0)">关注度
								</a>
							</li>
							<li <c:if test="${queryCourse.order=='NEW'}">class="current bg-orange"</c:if>>
								<a title="最新" onclick="submitForm(3,'NEW')" href="javascript:void(0)">最新
								</a>
							</li>
							<li <c:if test="${queryCourse.order=='ASCENDING'||queryCourse.order=='DESCENDING'}">class="current bg-orange"</c:if>>
								<a title="价格" onclick="submitForm(4,'<c:if test="${not empty queryCourse.order}">${queryCourse.order }</c:if><c:if test="${empty queryCourse.order}">ONE</c:if>')" href="javascript:void(0)">价格<span><c:if test="${queryCourse.order=='ASCENDING' }">↑</c:if><c:if test="${queryCourse.order=='DESCENDING' }">↓</c:if></span>
								</a>
							</li>
						</ol>
					</section>
				...
				<!-- 公共分页 开始 -->
	<jsp:include page="/WEB-INF/views/common_/front_page.jsp"></jsp:include>
				<!-- 公共分页 结束 -->
				<form action="${ctx_}/front/showcoulist" id="searchForm" method="post">
