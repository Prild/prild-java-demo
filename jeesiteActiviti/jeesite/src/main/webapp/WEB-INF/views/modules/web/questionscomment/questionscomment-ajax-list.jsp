						...
						<li>
							<aside class="noter-pic">
								<c:choose>
									...
									<c:otherwise>
										<img src="${ctx_s }/static_/inxweb/img/avatar-boy.gif" alt="" width="50" height="50">
									</c:otherwise>
								</c:choose>
							</aside>
							<div class="of hLh20">
							...
							<div class="of mt5">
								<span class="fr"><font class="fsize12 c-999 ml5"><fmt:formatDate type="both" value="${questionsComment.addTime }" pattern="yyyy-MM-dd HH:mm" /></font></span> <span class="fl"> <a href="javascript: void(0)" title="回答" class="noter-dy vam"
										onclick="getCommentById(this,${questionsComment.id_ })">
										<em class="icon18">&nbsp;</em>全部评论(<span>${questionsComment.replyCount }</span>)
									</a> <tt class="noter-zan vam ml10 f-fM" title="赞一下" onclick="addPraise(${questionsComment.id_ },2,this)">
							...
							<div class="n-reply">
								...
								<input type="hidden" id="" name="" value="${questionsComment.id_}" />
...
<div class="q-i-noter-box">
	...
	<c:if test="${not empty questionsCommentList  }">
						...
						<li>
							...
							<div class="of hLh20">
								<c:if test="${questions.status==0 and  questions.cusId==user.userId and questionsComment.cusId != questions.cusId}">
									<span class="fr"><a href="javascript:void(0)" onclick="acceptComment(${questionsComment.id_})" title="" class="comm-btn c-btn-6">采纳为最佳答案</a></span>
								</c:if>
								<span class="fl"> <font class="fsize12 c-blue"> <c:if test="${not empty questionsComment.showName}">${questionsComment.showName}</c:if> <c:if test="${empty questionsComment.showName}">${questionsComment.email}</c:if>
								</font> <font class="fsize12 c-999 ml5">回复：</font></span>
							</div>
							...
							<div class="of mt5">
								...
								</font></span> <span class="fl"> <a href="javascript: void(0)" title="回答" class="noter-dy vam" onclick="getCommentById(this,${questionsComment.id_})">
										<em class="icon18">&nbsp;</em>(<span>${questionsComment.replyCount }</span>)
									</a> <tt class="noter-zan vam ml10" title="赞一下" onclick="addPraise(${questionsComment.id_ },2,this)">${questionsComment.id_ }questionscomment-ajax-list.jsp
										<em class="icon18">&nbsp;</em>(<span>${questionsComment.praiseCount }</span>)${questionsComment.praiseCount }
									</tt>
								</span>
							</div>
							...
							<div class="n-reply" style="display: none;padding-left: 0;">
								<div>
									...
									<input type="hidden" id="" name="questionsComment.id_" value="${questionsComment.id_}" />
								</div>
								<div class="mt10 pl10 pr10">
									<dl class="n-reply-list">
										<!-- 当前问答回复的所有评论在这里 -->
		...
		<!-- 公共分页 开始 -->
	<jsp:include page="/WEB-INF/views/common_/ajaxpage.jsp"></jsp:include>
		<!-- 公共分页 结束 -->
