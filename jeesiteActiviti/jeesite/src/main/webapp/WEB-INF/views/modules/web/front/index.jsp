...
<body>
			...
			<section class="container">
				...
				<div>
					...
					<section class="tac pt20">
						<a href="${ctx_ }/front/showcoulist" title="全部课程" class="comm-btn c-btn-2">全部课程</a>
					</section>
				</div>
			</section>
		</div>
										...
										<section class="i-q-l-wrap">
											<div class="u-face">
												<c:if test="${empty comment.picImg }">
													<img width="50" height="50" class="picImg" src="${ctx }/static_/inxweb/img/avatar-boy.gif" xsrc="${ctx }/static_/inxweb/img/avatar-boy.gif">
												</c:if>
												<c:if test="${not empty comment.picImg }">
													<img width="50" height="50" class="picImg" src="${ctx }/static_/inxweb/img/avatar-boy.gif" xsrc="${ctx }${comment.picImg }">
												</c:if>
											</div>
											...
											<section class="hLh20 of mt10">
												<a class="c-master fsize14" href="${ctx_}/front/couinfo/${comment.courseId}">《${comment.courseName}》</a>
											</section>
		...
		<div>
			<section class="container">
				...
				<div>
								...
								<li>
									<section class="i-teach-wrap">
										<div class="i-teach-pic">
											<a href="${ctx_}/front/teacher/${teacher.id_}" title="${teacher.name}">
												<c:if test="${not empty teacher.picPath&&teacher.picPath!=''}">
													<img alt="${teacher.name}" src="${ctx}/static_/inxweb/img/default-tea-img.gif" xsrc="<%=staticImage%>${teacher.picPath}">
												</c:if>
												<c:if test="${empty teacher.picPath||teacher.picPath==''}">
													<img alt="${teacher.name}" src="${ctx}/static_/inxweb/img/default-img.gif" xsrc="${ctx}/static_/inxweb/img/default-img.gif">
												</c:if>
											</a>
										</div>
										<div class="mt10 hLh30 txtOf tac">
											<a href="${ctx_}/front/teacher/${teacher.id}" title="${teacher.name}" class="fsize18 c-666">${teacher.name}</a>
										</div>
					...
					<section class="tac pt20">
						<a href="${ctx_}/front/teacherlist" title="全部讲师" class="comm-btn c-btn-2">全部讲师</a>
					</section>
				</div>
			</section>
		</div>
	...
	<script type="text/javascript" src="${ctxStatic_}/inxweb/js/swiper-2.1.0.js"></script>
	<script type="text/javascript" src="${ctxStatic_}/inxweb/front/index.js"></script>
	<script type="text/javascript" src="${ctxStatic_}/inxweb/front/index_theme_color.js"></script><!-- 换肤 -->
	<script>
		$(function() {
			sSwiperFun(); //幻灯片调取
