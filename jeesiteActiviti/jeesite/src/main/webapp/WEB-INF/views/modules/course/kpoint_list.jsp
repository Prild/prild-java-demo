...
<title>课程节点</title>
	...
	<link rel="stylesheet" type="text/css" href="${ctxStatic_}/kindeditor/themes/default/default.css" />
	<script type="text/javascript" charset="utf-8" src="${ctxStatic_}/common/webutils.js"></script>
	
<script type="text/javascript" src="${ctxStatic_}/others/admin/kpoint/kpoint.js"></script>
<script type="text/javascript" src="${ctxStatic_}/others/admin/teacher/select_teacher_list.js"></script>

<script type="text/javascript" src="${ctxStatic_}/common/uploadify/ccswfobject.js"></script>

<script type="text/javascript" src="${ctxStatic_}/common/uploadify/swfobject.js"></script>
<script type="text/javascript" src="${ctxStatic_}/common/uploadify/jquery.uploadify.v2.1.4.min.js"></script>
<style type="text/css">
#swfDiv embed {
	position: absolute;
	z-index: 1;
...
var ztree='${kpointList}';
		$(function(){
			showKpointZtree(ztree);// kpoint.js
			
			initKindEditor_addblog('content', 580, 350,'courseContxt','true');//webutils.js文本编辑框

		});


		//上传控件加载 kpoint.js -->uploadPicLoad('fileupload','videourl','fileQueue');
		function uploadPicLoad(fileupload,showId,fileQueue){
			$("#fileuploadUploader").remove();
			$("#"+fileupload).uploadify({
				'uploader' : '/static/common/uploadify/uploadify.swf', //服务器端脚本文件路径，必写项，flash控件  默认值='uploadify.swf'
				<%--'script'  :'<%=uploadServerUrl%>/video/uploadvideo',  --%>
				'scriptData':{"fileType":"mp4","param":"video"},  //  http://127.0.0.1:38080/video/uploadvideo?fileType=mp4&param=video
				'queueID' : fileQueue, //文件队列ID=div ID
				'fileDataName' : 'uploadfile', //具体文件
				...
				onComplete : function(event,queueID, fileObj, response,data) {
					var obj = eval('(' + response + ')');//把json包裹成对象，调用里面的值
					$("#"+showId).val(obj.url);
					$("#"+showId).show();
				},
				...
				<fieldset style="height: 662px;">
					...
					<div class="mt20">
						<div class="commonWrap">
							...
							<a title="创建视频节点" onclick="addaKpoint(${courseId});" class="btn  btn-sm">
								创建视频节点
							</a>
							<a title="取消选中" onclick="ztreeCancelSelectedNode();" class="btn btn-sm" >
								<span></span>
								取消选中
							</a>
						</div>
						...
						<div class="commonWrap">
							<form id="updateForm">
								...
								<table style="line-height: 35px;" width="100%" cellspacing="0" cellpadding="0" border="0" class="commonTab01">
									...
									<tr>
										<td colspan="2">
											<button  class="btn btn-sm"  style="float: left;" onclick="updateKpoint()" type="button">确定</button>
											<button  class="btn btn-sm" style="float: left;" type="button">取消</button>
										</td>
									</tr>
								</table>
							</form>
