...
function toAddQuestions(){
	if(isLogin()){
		window.location.href=ctx_+"/questions/toadd";
	}else{
		lrFun();
	}
}
...
function submitForm(keyWord,type){
	if(type=="order"){
		if("status0"==keyWord){
	//等待回答：submitForm('status0','order')
			$("input[name='questions.status']").val(0);
			$("input[name='questions.orderFalg']").val(keyWord);
		}else{
	//最新：submitForm('addTime','order')
	//热门：submitForm('replycount','order')
			$("input[name='questions.orderFalg']").val(keyWord);
		}
	
	//"全部回答："submitForm(0,'type') "课程回答"：submitForm(1,'type') "学习分享"：submitForm(2,'type')
	}else if(type=="type"){
...
function hotQuestions(){
	$.ajax({
		url:ctx_ + "/questions/ajax/hotRecommend",
		data:{
		},
		type:"post",
		dataType:"json",
			...
			if(result.success==true){
				...
				for(var i=0;i<resultList.length;i++){
					str+='<li>'
						...
						+'		<a href="/jeesite/f/questions/info/'+resultList[i].id_+'" title="" class="fsize14 c-333 ml5">'+resultList[i].title+'</a>'
						+'	</h4>'
						+'</li>';
				}
