...
function queryUnReadNum(){
	...
	$.ajax({
		type : "POST",
		dataType : "json",
		url:ctx_+"/uc/ajax/queryUnReadLetter",
		cache : true,
		async : true,
		success : function(result) {
                    ...
                    '<div>'+
                        '<ol class="e-login-options">'+
                            ...
                            '<li>'+
                                ...
                                '<a href="javascript:void(0)" title="" class="vam ml10 disIb fl"><img onclick="this.src=\'/jeesite/f/ran/random?random=\'+Math.random()" alt="验证码，点击图片更换" src="/jeesite/f/ran/random" width="86" height="40"></a>'+
	...
	$.ajax({
		...
		success:function(result){
			if(result.success==false){
				...
				if(type==1){
					//console.info(baselocation);
					window.location.href= ctx_+"/uc/index";
				}else{
...
function getCourseLearnedUser(courseId){
	$.ajax({
		type : "POST",
		dataType : "json",
		url:ctx_+"/couserStudyHistory/ajax/courseLearnedUser/"+courseId,
		cache : true,
		async : false,
		success : function(result) {
			if(result.success==true){
					...
					var useImg="";
	 				...
	 				for(var i=0;i<resultList.length;i++){
	 					...
	 					if(useImg==null || $.trim(useImg)==''){
	 						useImg = ctx_s+'/static_/inxweb/img/avatar-boy.gif';
	 					}else{
