...
function getPlayerHtml(kpointId,name,obj) {
	...
	$.ajax({
		url : "" + ctx_ + "/front/ajax/getKopintHtml",
		data : {
			"kpointId" : kpointId,
			"courseId" : otherId
		},
...
function favorites(courseId,obj) {
	$.ajax({
		url : ctx_ + '/front/createfavorites/' + courseId,
		type : 'post',
		dataType : 'json',
		success : function(result) {
 ...
 function addNotest() {
 	...
 	if (KindEditor.get("#notesContextId").isEmpty()) {
 		...
 		$.ajax({
 			url : ctx_ + "/courseNote/ajax/addnote",
 			type : 'post',
 			dataType : 'json',
 			data : {
...
function queryNote() {
	$.ajax({
		url : ctx_ + "/courseNote/ajax/querynote",
		type : "post",
		dataType : "json",
		data : {
			'kpointId' : currentKpointId,
...
function addPlayTimes(courseId, kpointId) {
	$.ajax({
		url : ctx_ + "/couserStudyHistory/ajax/playertimes",
		data : {
			"kpointId" : kpointId,
			"courseId" : courseId
		},
