...
function initUpdateKpoint(treeId, treeNode){
	closeData();//把右边修改的内容清空并隐藏起来，重新加载
	var treeNodeLeve=treeNode.level;
	var childrenNodes = treeNode.children;
	
	$.ajax({
		url:ctx+'/kpoint/getkpoint/'+treeNode.kpointId,
		type:'post',
		dataType:'json',
		success:function(result){
...
function updateKpoint(){
	...
	params+="courseKpoint.kpointType="+kpointType+"&";//节点类型:目录/章节
	
	var videoType=$("#courseKpointVideoType").val();
	params+="courseKpoint.videoType="+videoType+"&";//视频类型
	
	var fileType=$("#fileType").val();
	params+="courseKpoint.fileType="+fileType+"&";//课件类型:VIDEO视频 AUDIO音频 FILE文档 TXT文本 ATLAS图片集
	$.ajax({
		url:ctx+'/kpoint/updateKpoint',
		type:'post',
		dataType:'json',
		data:params,
		success:function(result){
...
function deleteKpoint(treeId, treeNode){
	...
	getChildren(treeNode);//删除选中的节点及其下所有子节点
	$.ajax({
		url:ctx+'/kpoint/deletekpoint/'+ids,
		type:'post',
		dataType:'json',
		async:false,
		success:function(result){
...
function updateKpointParentId(treeId, treeNodes, targetNode, moveType){
	...
	var is = true;
	$.ajax({
		url:ctx+'/kpoint/updateparentid/'+parentId+'/'+kpointId,
		type:'post',
		async:false,
		dataType:'json',
		success:function(result){
...
function getKpointType(parentId){
	var isTrue=true;
	$.ajax({
		url:ctx+'/kpoint/getkpoint/'+parentId,
		type:'post',
		dataType:'json',
		async:false,
		success:function(result){
