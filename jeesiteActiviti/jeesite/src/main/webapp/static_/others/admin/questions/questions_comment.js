...
function delQuestionsComment(commentId){
	if(confirm("是否删除此问答回复,及问答回复下的所有评论吗?")){
		$.ajax({
			url:ctx+"/questionscomment/del/"+commentId,
			data:{
			},
			dataType:"json",
			type:"post",
...
function acceptComment(id){
	$.ajax({
		url:ctx+"/questionscomment/ajax/acceptComment/"+id,
		data:{
		},
		type:"post",
		dataType:"json",
...
function getCommentContent(questionsCommentId){
	//closedData();
	$.ajax({
		url:ctx+'/questionscomment/getCommentById/'+questionsCommentId,
		type:'post',
		dataType:'json',
		success:function(result){
			if(result.success==true){
				var questionsComment = result.entity;
				$("#updateForm input[name='questionsComment.id_']").val(questionsComment.id_);
...
function updateQuestionComment(){
	...
	var id_=$("#updateForm input[name='questionsComment.id_']").val();
	var praiseCount=$("#updateForm input[name='questionsComment.praiseCount']").val();
	$.ajax({
		url:ctx+"/questionscomment/updQuestionComment",
		data:{
			"questionsComment.id_":id_,
...
function winHide(){
	...
	//$("input:text").val('');
	//$("input:textarea").val('');
	//$("#updateWin name='[questionsComment.id_]'").val(0);
}
