...
function updateParentId(treeId, treeNodes, targetNode, moveType) {
	...
	var is = true;
	$.ajax({
		url : ctx + '/subj/updateparentid/' + parentId + '/' + subjectId,
		type : 'post',
		async : false,
		dataType : 'json',
		success : function(result) {
...
function updateSubject(treeId, treeNode, newName, isCancel) {
	...
	$.ajax({
		url : ctx + '/subj/updatesubjectName',
		type : 'post',
		async : false,
		dataType : 'json',
		data : {
		...
		success : function(result) {
			if (result.success == false) {
				alert(result.message);
				document.location = ctx + '/subj/toSubjectList';
			}
		},
		error : function(error) {
			alert("系统繁忙，请稍后再操作！");
...
function deleteSubject(treeId, treeNode) {
	...
	if (confirm('确认要删除[' + treeNode.subjectName + ']专业？')) {
		$.ajax({
			url : ctx + '/subj/deleteSubject/' + treeNode.subjectId,
			type : 'post',
			async : false,
			dataType : 'json',
			success : function(result) {
