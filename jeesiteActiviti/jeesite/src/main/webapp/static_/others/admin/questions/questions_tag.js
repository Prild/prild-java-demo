...
$(function(){
	var questionsTagList_ = eval('('+questionsTagList+')');
	var treeObj = $.fn.zTree.init($("#ztreedemo"), setting, questionsTagList_);
...
function addQuestionsTag(){
	...
	parentId=0;//只添加一级
	$.ajax({
		url:ctx+'/questions/createQuestionsTag',
		type:'post',
		dataType:'json',
...
function updateParentId(treeId, treeNodes, targetNode, moveType){
	...
	var is = true;
	$.ajax({
		url:ctx+'/questions/updateparentid/'+parentId+'/'+questionsTagId,
		type:'post',
		async:false,
		dataType:'json',
		success:function(result){
...
function updateQuestionsTag(treeId, treeNode, newName, isCancel){
	...
	$.ajax({
		url:ctx+'/questions/updatequestionsTagName',
		type:'post',
		async:false,
		dataType:'json',
		...
		success:function(result){
			if(result.success==false){
				alert(result.message);
				document.location=ctx+'/questions/toQuestionsTagList';
			}
		},
		error:function(error){
			alert("系统繁忙，请稍后再操作！");
...
function deleteQuestionsTag(treeId, treeNode){
	...
	if(confirm('确认要删除['+treeNode.questionsTagName+']问答标签？')){
		$.ajax({
			url:ctx+'/questions/deleteQuestionsTag/'+treeNode.questionsTagId,
			type:'post',
			async:false,
			dataType:'json',
			success:function(result){
