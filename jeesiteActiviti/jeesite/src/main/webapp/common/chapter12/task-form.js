$(function() {
	...
	readComments();// 读取事件列表(优先加载)

	
	$('#saveComment').click(function() {// 保存意见 (单独保存comment)
		if(!$('#comment').val()) {
			return false;
		}
		$.post(ctx + '/act/task4/comment/save', {
			taskId: taskId,
	...
	taskProperties();// 处理任务属性
	
	delegate();// 任务委派
	
	participants();// 参与人（添加、删除）
...
function taskProperties() {
	...
	$('.due-date-input').blur(function() {
		...
		var value = $(this).val();
		if(value) {
			$.post(ctx + '/act/task2/property/' + taskId, {
...
function participants() {
	...
	$('#sendAddParticipantsRequest').click(function() {
		...
		$.post(ctx + '/act/task2/participant/add/' + taskId, {
			userId: userIds,
			type: types
		}, function(resp) {
			if(resp == 'success') {
	...
	$('.link-delete').click(function() {
		var ele = this;
		$.post(ctx + '/act/task2/participant/delete/' + taskId, {
...
function candidate() {
	...
	$('#sendAddCandidateRequest').click(function() {
		...
		$.post(ctx + '/act/task2/candidate/add/' + taskId, {
...
function subTask() {
	$('.subtask-delete').click(function() {
		...
		location.href = ctx + '/act/task2/delete/' + $(this).data('taskid');
	});
}
...
function attachment() {
	...
	$('.attachment-delete').click(function() {
		var ele = this;
		if(confirm('确定删除附件？')) {
			$.get(ctx + '/act/task3/attachment/delete/' + $(this).data('id'), function(resp) {
				if(resp == 'true') {
					$(ele).parent().remove();
				} else {
					alert('删除失败！');
...
function delegate() {
    ...
    $('#delegateUserSelect').change(function() {
        ...
        var value = $(this).val();
        if(value) {
            $.post(ctx + '/act/task2/delegate/' + taskId, {
