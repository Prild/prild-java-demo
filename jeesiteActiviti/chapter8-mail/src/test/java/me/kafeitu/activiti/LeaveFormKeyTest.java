...
public class LeaveFormKeyTest extends AbstractTest {
    ...
    public void allPass() throws Exception {
        ...
        Calendar ca = Calendar.getInstance();//今天
        String startDate = sdf.format(ca.getTime());
        ca.add(Calendar.DAY_OF_MONTH, 2); // 今天+2天
        ...
        ProcessDefinition processDefinition = repositoryService.createProcessDefinitionQuery().processDefinitionKey("leave-formkey").singleResult();
        ...
        identityService.setAuthenticatedUserId(currentUserId);//identityService操作ACT_ID_*表
        ProcessInstance processInstance = formService.submitStartFormData(processDefinition.getId(), variables);//formService处理表单数据
        assertNotNull(processInstance);

        // 部门领导审批通过
        Task deptLeaderTask = taskService.createTaskQuery().taskCandidateGroup("deptLeader").singleResult();//taskService处理任务
        assertNotNull(formService.getRenderedTaskForm(deptLeaderTask.getId()));
        variables = new HashMap<String, String>();
        variables.put("deptLeaderApproved", "true");//部门领导同意
        ...
        // 验证流程是否已经结束
        HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery().finished().singleResult();//historyService历史服务在流程实例执行的过程中,会不断触发事件。 从那些事件中,运行和完成流程的历史信息会被收集到历史表中
        ...
        // 验证执行结果
        assertEquals("ok", historyVariables.get("result"));//表单中字段没有result
