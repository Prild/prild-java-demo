...
public class PurchaseCallActivityTest extends SpringActivitiTestCase {
    ...
    public void testAllApproved() throws Exception {
      ...
      //purchase-callactivity.bpmn办公用品采购--callactivity方式
        ...
        // 验证是否启动子流程
        Execution subExecution = runtimeService.createExecutionQuery().processDefinitionKey("payment").singleResult();//payment.bpmn 通用付款流程
    ...
    public void testRejectOnTreasurer() throws Exception {
        ...
        // 验证是否启动子流程
        Execution subExecution = runtimeService.createExecutionQuery().processDefinitionKey("payment").singleResult();//payment.bpmn 支付子流程
        assertNotNull(subExecution);
        assertEquals(listing, runtimeService.getVariable(subExecution.getId(), "usage"));
