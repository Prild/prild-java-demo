...
public class UserController extends BaseController {
	...
	/**
	 * 修改头像 (流程：登录进来根据当前id查询头像并显示->点击头像进入 userController.do?changeportrait (jsp页面tags由类处理)->点击保存 userController.do?saveportrait 流程结束)
	 * @param request
	 * @return
	 */
	...
	@RequestMapping(params = "saveportrait")//changeportrait.jsp 页面 userController.do?saveportrait
	@ResponseBody
	public AjaxJson saveportrait(HttpServletRequest request,String fileName) {
		AjaxJson j = new AjaxJson();
		TSUser user = ResourceUtil.getSessionUser();
		TSBaseUser u = ResourceUtil.getSessionUser();
